...
class WBIKEY {
private:
	static constexpr  std::array<uint8_t, 64> MIXIN_KEY_ENC_TAB_ = {
...
private:
	static std::string GetMd5Hex(const std::string& inputStr);
	static std::string JsonToUrlEncodeStr(const nlohmann::json& Json);
public:
	static std::string CalcSign(nlohmann::json Params, const std::string& wbi);
